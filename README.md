...
An example CRAB config for the gridpack to LHE step is located in [crab_LHE_s.py](FullSim_files/CRAB/crab_LHE_s.py). The comments should be detailed enough, with the CMSSW config being specified by `config.JobType.psetName` and being created as it would be when running locally. When ready to submit the CRAB job(s), you will first have to source the CMSSW release used to make the CMSSW config, and then set up the CRAB environment with
