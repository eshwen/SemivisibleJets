...
if [ -z $1 ]; then
    echo "--------------------------------------------------------------------------------------------------------
Usage ./submitFullSim_condor.sh work_space lhe_file_path model_name n_events n_jobs Lambda_d config_file
--------------------------------------------------------------------------------------------------------"
    exit
fi
...
for seed in $(seq 0 1 $(( $n_jobs-1 ))); do
    ...
    job_path=$($submission_dir/writers/write_submission_script.sh $work_space $gen_frag_file $lhe_file_for_job $model_name $n_events $seed $submission_dir)
